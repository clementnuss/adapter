        ...
        if (mIsService) {
                ...
                if (strcmp(aArgv[i], "-c") == 0 || strcmp(aArgv[i], "--conf") == 0 && (i + 1 < aArgc)) {
                    for (int j = 0; j < strlen(aArgv[i + 1]); j++) {
                        if ((strncmp(&aArgv[i + 1][j], "/", 1) == 0)) {  //strip the path (if any) from the config file,
                            start = j + 1;
                        } else if (strncmp(&aArgv[i + 1][j], ".", 1) ==
                                   0) {  // strip the file extension (typically ".ini" but just strip the last "." and everything after it)
                            end = j - 1;
                        }
                    }
            ...
            if (gLogger == NULL) {
                if (access("/var/log/adapter", W_OK) == 0) {
                    ...
                    gLogger = new Logger();
                    gLogger->error(
                            "Unable to write log file in the /var/log/adapter/ folder, please check permissions");
                }
            }
        }
...
VOID SvcReportEvent(LPTSTR szFunction) {
    ...
    if (NULL != hEventSource) {
        sprintf
        (Buffer, "%-60s failed with %d", szFunction, GetLastError());
